...
export class AchievementCalculator implements IAchievementCalculator {
    ...
    for (const milestone of streakMilestones) {
      if (currentStreak >= milestone.days) {
        const existingAchievement = existing.find(
          a => a.type === 'streak' && a.criteria.threshold === milestone.days
        );
  ...
  private checkVolumeAchievements(
    statistics: ProfileStatistics,
    existing: Achievement[]
  ): Achievement[] {
    const achievements: Achievement[] = [];
    ...
    for (const milestone of volumeMilestones) {
      if (totalQuestions >= milestone.count) {
        const existingAchievement = existing.find(
          a => a.type === 'volume' && a.criteria.threshold === milestone.count
        );
  ...
  private checkAccuracyAchievements(
    statistics: ProfileStatistics,
    existing: Achievement[]
  ): Achievement[] {
    const achievements: Achievement[] = [];
    ...
    for (const milestone of accuracyMilestones) {
      if (accuracy >= milestone.accuracy) {
        const existingAchievement = existing.find(
          a => a.type === 'accuracy' && a.criteria.threshold === milestone.accuracy
        );
  ...
  private checkMilestoneAchievements(
    ...
    for (const milestone of sessionMilestones) {
      if (totalSessions >= milestone.sessions) {
        const existingAchievement = existing.find(
          a => a.type === 'milestone' && a.criteria.threshold === milestone.sessions
        );
