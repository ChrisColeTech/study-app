...
class QuestionService {
    ...
    async getQuestions(request) {
        ...
        if (request.topics && request.topics.length > 0) {
            filteredQuestions = filteredQuestions.filter(q => request.topics.includes(q.question.topic));
        }
        // Apply difficulty filter
        if (request.difficulty) {
            filteredQuestions = filteredQuestions.filter(q => q.study_metadata.difficulty === request.difficulty);
        }
        // Apply pagination
    ...
    async getQuestionById(provider, exam, questionId) {
        const questions = await this.loadQuestions(provider, exam);
        return questions.find(q => q.question_number.toString() === questionId) || null;
    }
    async loadQuestions(provider, exam) {
        try {
            const client = aws_clients_1.default.getS3Client();
    ...
    async getAvailableTopics(provider, exam) {
        const questions = await this.loadQuestions(provider, exam);
